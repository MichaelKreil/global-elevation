...
export class Block {
	...
	static fromGeoTiff(buffer) {
		...
		function getIFD(offsetIFD) {
			let sliceIFD = bufferWrapper(buffer.slice(offsetIFD));
			let count = sliceIFD.getUInt16(0);
			let ifdEntries = new Array(count);
			let tags = {};

			for (let i = 0; i < count; i++) {
				let sliceEntry = bufferWrapper(sliceIFD.slice(i*12+2));
			...
			let ArrayType, converter, bytesPerSample;
			...
			for (let stripIndex = 0; stripIndex < tags.stripOffsets.length; stripIndex++) {
				...
				let strip = buffer.slice(stripOffset, stripOffset+stripByteCount);
		...
		function processIFDEntryTag(entry, tags) {
			...
			function getArray() {
				...
				switch (entry.type) {
					case  2:
						return buffer.slice(entry.offset, entry.offset + entry.count).toString();
					case  3:
		...
		function getProjection(tags) {
			...
			if (tags.modelTransformation) throw Error('not implemented');
			if (tags.modelPixelScale.length !== 3) throw Error('not implemented');
			if (tags.modelTiepoint.length !== 6) throw Error('not implemented');
			if (tags.orientation !== 1) throw Error('not implemented');
			if (tags.modelTiepoint[0] !== 0) throw Error('not implemented');
			if (tags.modelTiepoint[1] !== 0) throw Error('not implemented');
