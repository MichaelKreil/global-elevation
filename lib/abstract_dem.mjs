...
export class AbstractDEM {
	...
	constructor(cacheFolder) {
		this.#cacheFolder = cacheFolder ?? resolve(CACHE_FOLDER, this.constructor.name);
	...
	async getElevation(lng, lat) {
		...
		block.counter++;
		return block.getElevation(lng, lat);
	}
...
export class Block {
	...
	static load(filename) {
		...
		let ArrayType;
		switch (header.type) {
			case      'Int8Array': ArrayType =      Int8Array; break;
			case     'Uint8Array': ArrayType =     Uint8Array; break;
			...
			case   'Float32Array': ArrayType =   Float32Array; break;
			case   'Float64Array': ArrayType =   Float64Array; break;
			case  'BigInt64Array': ArrayType =  BigInt64Array; break;
			case 'BigUint64Array': ArrayType = BigUint64Array; break;
			default:
				console.log({tags});
				throw Error('Block: Unknown header.type '+header.type);
		}
	...
	static fromGeoTiff(buffer) {
		...
		function getIFD(offsetIFD) {
			...
			for (let i = 0; i < count; i++) {
				...
				if ((i > 0) && (ifdEntries[i].tag <= ifdEntries[i-1].tag)) throw Error('GeoTiff: tags are in wrong order');
			}

			if ((tags.samplePerPixel > 1) && (tags.planarConfiguration > 1)) throw Error('GeoTiff: not implementation');
			...
			switch (tags.sampleFormat+'/'+tags.bitsPerSample) {
				...
				default:
					console.log({tags});
					throw Error('GeoTiff: not implementation for tags.sampleFormat = '+tags.sampleFormat+', tags.bitsPerSample = '+tags.bitsPerSample);
			}
			...
			for (let stripIndex = 0; stripIndex < tags.stripOffsets.length; stripIndex++) {
				...
				switch (tags.compression) {
					...
					default: console.log({tags}); throw Error('GeoTiff: not implementation for compression = '+tags.compression);
				}
		...
		function processIFDEntryType(entry) {
			switch (entry.type) {
				...
				default: throw Error('GeoTiff: not implementation for entry.type = '+entry.type)
			}
		}
		...
		function processIFDEntryTag(entry, tags) {
			...
			switch (entry.tag) {
				...
				case 42112: return tags.gdalMetadata = getArray();
				case 42113: return tags.gdalNodata = getArray();
